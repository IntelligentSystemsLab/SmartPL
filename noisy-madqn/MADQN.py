...
class MADQN:
    ...
    def __init__(self,
        ...
        self.env_state = np.asarray(self.env_state).reshape(
            (len(self.env_state), -1))
        self.n_episodes = 0
        self.n_steps = 0
        ...
        self.target_network = ActorNetwork(self.state_dim,
        ...
        if self.optimizer_type == "adam":
            self.q_netwok_optimizer = Adam(self.q_network.parameters(),
                                           lr=self.actor_lr)
        elif self.optimizer_type == "rmsprop":
            self.q_netwok_optimizer = RMSprop(self.q_network.parameters(),
                                              lr=self.actor_lr)
        if self.use_cuda:
            self.q_network.cuda()
    ...
    def interact(self):
        ...
        for agent_id in range(self.n_agents):
            self.memory.push(state[agent_id], action[agent_id],
                             reward[agent_id], next_state[agent_id], done)
            # self.memory.push(state[agent_id, :], action[agent_id],
            #                  reward[agent_id], next_state[agent_id, :], done)
    ...
    def train(self):
        ...
        for _ in range(10):
            ...
            rewards_var = to_tensor_var(batch.rewards,
                                        self.use_cuda).view(-1, 1)
            next_states_var = to_tensor_var(batch.next_states,
                                            self.use_cuda).view(
                                                -1, self.state_dim)
            ...
            next_state_action_values = self.target_network(
                next_states_var).detach()
            ...
            if self.max_grad_norm is not None:
                nn.utils.clip_grad_norm(self.q_network.parameters(),
                                        self.max_grad_norm)
    ...
    def action(self, state):
        ...
        # state_var = th.from_numpy(state).to('cuda').unsqueeze(0)
    ...
    def evaluation(self, env, eval_episodes=10):
        ...
        for i in range(eval_episodes):
            ...
            reward_metrics_i = []
            state, _ = env.reset()
            state = np.asarray(state).reshape((len(state), -1))
            action = self.action(state)
            state, reward, done, info = env.step(tuple(action))
            done = done[0] if isinstance(done, list) else done
            rewards_i.append(reward)
            reward_metric = np.mean(list(info['reward_metric'].values()))
            # reward_metric = info['reward_metric']['cav_0']
            reward_metrics_i.append(reward_metric)
            infos_i.append(info)
            while not done:
                action = self.action(state)
                state, reward, done, info = env.step(tuple(action))
            ...
            reward_metrics.append(reward_metrics_i)
            infos.append(infos_i)
            
        # env.close()
        return rewards, infos,reward_metrics
    ...
    def load(self, model_dir, global_step=None, train_mode=False):
        ...
        if save_file is not None:
            ...
            if train_mode:
                self.q_netwok_optimizer.load_state_dict(
