...
# extractors
# EXTRACTORS = ['Graph','Kinematic','OccupancyGrid']
EXTRACTORS = ['Graph','OccupancyGrid']
...
for lane_count in LANE_COUNT: 
    for hdv_interval in HDV_INTERVAL:
        for extractor in EXTRACTORS:
            if  extractor == 'Graph':
                env_name = 'PLDriving_highway_v2_{}'.format(extractor)
                ...
                print("\n============Loading {} {} Model===========\n".format(
                    model.__class__.__name__, env_name))
                model_path = os.path.join(r'checkpoints\Graph_MaskablePPO\PLDriving_highway_v2_Kinematic\PPO_2\best_model.zip')
                model = model.load(model_path, env=env)
                print("\n============Loading {} {} Model===========\n".format(
                    model.__class__.__name__, env_name))
            else:
                ...
                print("\n============Loading {} {} Model===========\n".format(
                    model.__class__.__name__, env_name))
                model_path = os.path.join(r'checkpoints\Graph_MaskablePPO\PLDriving_highway_v2_OccupancyGrid\PPO_1\best_model.zip')
                model = model.load(model_path, env=env)
                print("\n============Loading {} {} Model===========\n".format(
                    model.__class__.__name__, env_name))
...
# 保存为 CSV 文件
# df.to_csv('data/extractors/interval{0}/extrators_{1}.csv'.format(HDV_INTERVAL[0],SEED), index=False)
